      ...
      >
        <div className="container mx-auto flex items-center justify-between px-4 sm:px-6 max-w-full">
          ...
          <div className="shrink-0">
            <NavLink to="/">
              <img src={logo} alt="Noor Logo" className="h-5 md:h-8  w-auto object-contain" />
            </NavLink>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-6 xl:gap-8 text-white">
            {links.map((link) => (
              <NavLink 
                key={link.path} 
                to={link.path} 
                className={({ isActive }) => `text-[13px] xl:text-[14px] font-semibold transition-all whitespace-nowrap ${isActive ? "text-primary" : "text-white/80 hover:text-primary"}`}
              >
                {link.name}
              </NavLink>
            ))}
